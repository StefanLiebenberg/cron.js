...
sage.cron.Job.prototype.run=function(){this.last_at=new Date;setTimeout(this.block_,10);this.calcNextAt_()};sage.cron.Scheduler=function(){this.jobs_=[];this.running_=!1;this.stored_timeout_=void 0};sage.cron.Scheduler.prototype.add=function(a){this.jobs_.push(a);this.running_&&this.next()};sage.cron.Scheduler.prototype.tolerance_=1E3;sage.cron.Scheduler.prototype.check=function(){this.running_&&this.next_scheduled_at_<new Date-this.tolerance_&&this.restart()};sage.cron.Scheduler.prototype.remove=function(a){for(var b;(b=this.jobs_.indexOf(a))!==-1;)this.jobs_.splice(b)};
sage.cron.Scheduler.prototype.start=function(){if(!this.running_)this.running_=!0,this.next(),this.startCheck_()};sage.cron.Scheduler.prototype.startCheck_=function(){if(!this.stored_interval_)this.stored_interval_=setInterval(this.check.bind(this),6E4)};sage.cron.Scheduler.prototype.stopCheck_=function(){this.stored_interval_&&clearInterval(this.stored_interval_)};sage.cron.Scheduler.prototype.restart=function(){this.stop();this.start()};
sage.cron.Scheduler.prototype.stop=function(){if(this.running_)this.running_=!1,this.clearTimeout_(),this.stopCheck_()};sage.cron.Scheduler.prototype.next=function(){var a=!this.running_;(a=(a=a||Boolean(this.stored_timeout_))||!Boolean(this.jobs_.length))||this.executeJob_(this.getNextJob())};
sage.cron.Scheduler.prototype.executeJob_=function(a){var b=new Date,c=a.getNextTimeout(null);this.next_scheduled_at_=new Date(b.getTime()+c);this.setTimeout_(function(){a.run();this.clearTimeoutValue_();delete this.next_scheduled_at_;this.next()},c)};sage.cron.Scheduler.prototype.setTimeout_=function(a,b){this.setTimeoutValue_(setTimeout(goog.bind(a,this),b))};sage.cron.Scheduler.prototype.clearTimeout_=function(){var a=this.getTimeoutValue_();clearTimeout(a);this.clearTimeoutValue_()};
sage.cron.Scheduler.prototype.setTimeoutValue_=function(a){this.stored_timeout_=a};sage.cron.Scheduler.prototype.getTimeoutValue_=function(){return this.stored_timeout_};sage.cron.Scheduler.prototype.clearTimeoutValue_=function(){delete this.stored_timeout_};sage.cron.Scheduler.prototype.getNextJob=function(){for(var a=this.jobs_[0],b,c=1,d=this.jobs_.length;c<d;c++)b=this.jobs_[c],b.next_at<a.next_at&&(a=b);return a};sage.util={};sage.util.Parser=function(){};sage.util.Parser.prototype.parse=function(a){return this.parseInternal.apply(this,arguments)};sage.util.Parser.prototype.parseInternal=function(a){return a};sage.util.StringParser=function(a){sage.util.Parser.call(this);this.regex=a};goog.inherits(sage.util.StringParser,sage.util.Parser);sage.util.StringParser.prototype.test=function(a){this.regex.lastIndex=0;return this.regex.test(a)};
sage.util.StringParser.prototype.parseInternal=function(a){return this.test(a)?(this.regex.lastIndex=0,a.match(this.regex)):[]};sage.cron.syntax={};sage.cron.syntax.AllParser=function(){sage.util.StringParser.call(this,/^\*$/)};goog.inherits(sage.cron.syntax.AllParser,sage.util.StringParser);sage.cron.syntax.AllParser.prototype.parseInternal=function(a,b){return b.range.getValues(null,null)};sage.cron.syntax.RangeParser=function(a){sage.util.StringParser.call(this,RegExp("^"+a+"-"+a+"$"))};goog.inherits(sage.cron.syntax.RangeParser,sage.util.StringParser);sage.cron.syntax.RangeParser.prototype.parseInternal=function(a,b){var c=a.split("-"),d=parseInt(c[0],10)-b.range.from,c=parseInt(c[1],10)-b.range.from;return b.range.getValues(null,null).slice(d,c+1)};sage.cron.syntax.IncrementParser=function(a){sage.util.StringParser.call(this,RegExp("^[^/]+/"+a+"$"))};goog.inherits(sage.cron.syntax.IncrementParser,sage.util.StringParser);sage.cron.syntax.IncrementParser.prototype.parseInternal=function(a,b){for(var c=a.split("/"),d=b.parse(c[0]),c=parseInt(c[1],10),e=[],i=0,j=d.length;i<j;i+=c)e.push(d[i]);return e};sage.util.Range=function(a,b){if(a>b)throw Error("sage.util.Range: from is larger than to");this.from=a;this.to=b;this.length=1+b-a};sage.util.Range.prototype.valueAt=function(a){return a<0||a>=this.length?void 0:this.from+a};sage.util.Range.prototype.indexOf=function(a){var b=-1;a>=this.from&&a<=this.to&&(b=a-this.from);return b};sage.util.Range.prototype.getValues=function(a,b){if(typeof a!=="number")a=this.from;if(typeof b!=="number")b=this.to;return this.getValuesInternal(a,b)};
sage.util.Range.prototype.getValuesInternal=function(a,b){var c,d;if(a>b||a<this.from||b>this.to)throw c="sage.util.Range: ",c+="values are out of range",Error(c);c=1+b-a;d=Array(c);if(c>0)for(;c--;)d[c]=a+c;return d};sage.util.Range.prototype.toString=function(){return"Range["+this.from+".."+this.to+"]"};sage.util.array={};sage.util.array.uniq=function(a){for(var b=a.length,c,d,e=0;e<b;e+=1){c=a[e];for(d=e+1;(d=a.indexOf(c,d))!==-1;)a.splice(d,1),b--}return a};sage.util.RangeParser=function(a,b){this.range=new sage.util.Range(a,b);this.parsers=[]};goog.inherits(sage.util.RangeParser,sage.util.Parser);sage.util.RangeParser.prototype.parseInternal=function(a){for(var b=[],c=this.parsers.length,d,e=0;e<c;e++)if(d=this.parsers[e],d.test(a)){b=d.parse(a,this);sage.util.array.uniq(b);b.sort(function(a,b){return a-b});break}return b};sage.cron.syntax.SingleParser=function(a){sage.util.StringParser.call(this,RegExp("^"+a+"$"))};goog.inherits(sage.cron.syntax.SingleParser,sage.util.StringParser);sage.cron.syntax.SingleParser.prototype.parseInternal=function(a,b){var c=parseInt(a,10)-b.range.from;return[b.range.valueAt(c)]};sage.cron.syntax.AliasParser=function(){sage.util.StringParser.call(this,/([a-z][a-z][a-z])|([A-Z][A-Z][A-Z])/g);this.aliases={}};goog.inherits(sage.cron.syntax.AliasParser,sage.util.StringParser);sage.cron.syntax.AliasParser.prototype.parseInternal=function(a,b){a=a.replace(this.regex,function(a){a=a.toLowerCase();a in b.aliases&&(a=b.aliases[a]);return a});return b.parse(a)};sage.cron.syntax.CommaParser=function(){sage.util.StringParser.call(this,/\,/)};goog.inherits(sage.cron.syntax.CommaParser,sage.util.StringParser);sage.cron.syntax.CommaParser.prototype.parseInternal=function(a,b){var c,d,e;c=a.split(",");e=c.length;d=[];for(var i=0;i<e;i++)d.push.apply(d,b.parse(c[i]));return d};sage.cron.SpecParser=function(a,b,c){sage.util.RangeParser.call(this,a,b);this.allow=c;this.parsers[0]=new sage.cron.syntax.CommaParser;this.parsers[1]=new sage.cron.syntax.AllParser;this.parsers[2]=new sage.cron.syntax.SingleParser(this.allow);this.parsers[3]=new sage.cron.syntax.RangeParser(this.allow);this.parsers[4]=new sage.cron.syntax.IncrementParser(this.allow)};goog.inherits(sage.cron.SpecParser,sage.util.RangeParser);
...
sage.cron.Spec.prototype.next=function(a){var b=new Date;b.setTime(a.getTime());sage.util.date.addSeconds(b,1);b.setMilliseconds(0);for(var c=(a=this.seconds.length)&&a!==60,d=this.minutes.length,e=d&&d!==60,i=this.hours.length,j=i&&i!==24,k=this.weekdays.length,o=k&&k!==7,l=this.days.length,p=l&&l!==31,m=this.months.length,q=m&&m!==12,n=!1,g,h;!n;)if(g=b.getSeconds(),c&&this.seconds.indexOf(g)===-1){h=!0;for(var f=0;f<a;f++)if(this.seconds[f]>=g){b.setSeconds(this.seconds[f]);h=!1;break}h&&(b.setSeconds(0),
sage.util.date.addMinutes(b,1))}else if(g=b.getMinutes(),e&&this.minutes.indexOf(g)==-1){h=!0;for(f=0;f<d;f++)if(this.minutes[f]>=g){b.setMinutes(this.minutes[f]);h=!1;break}h&&(b.setMinutes(0),sage.util.date.addHours(b,1))}else{g=b.getHours();if(j&&this.hours.indexOf(g)==-1){h=!0;for(f=0;f<i;f++)if(this.hours[f]>=g){b.setHours(this.hours[f]);h=!1;break}if(h){sage.util.date.addDays(b,1);b.setSeconds(0);b.setMinutes(0);b.setHours(0);continue}}g=b.getDay()+1;if(o&&this.weekdays.indexOf(g)==-1){h=!0;
for(f=0;f<k;f++)if(this.weekdays[f]>=g){sage.util.date.addDays(b,this.weekdays[f]-g-1);h=!1;break}if(h){sage.util.date.addDays(b,8-g);b.setSeconds(0);b.setMinutes(0);b.setHours(0);continue}}g=b.getDate();if(p&&this.days.indexOf(g)==-1){h=!0;for(f=0;f<l;f++)if(this.days[f]>=g){for(;b.getDate()!=this.days[f];)b.setDate(this.days[f]);h=!1;break}b.setSeconds(0);b.setHours(0);b.setMinutes(0);h&&(b.setDate(1),sage.util.date.addMonths(b,1))}else if(g=b.getMonth()+1,q&&this.months.indexOf(g)==-1){h=!0;for(f=
0;f<m;f++)if(this.months[f]>=g){b.setMonth(this.months[f]-1);h=!1;break}b.setSeconds(0);b.setMinutes(0);b.setHours(0);b.setDate(1);h&&(b.setMonth(0),sage.util.date.addYears(b,1))}else n=!0}return b};sage.cron.Spec.aliases_={"@second":"* * * * * *","@minute":"0 * * * * *","@halfminute":"0,30 * * * * *","@hour":"0 0 * * * *","@halfhour":"0 0,30 * * *","@daily":"0 0 0 * * *","@monthly":"0 0 0 1 * *","@yearly":"0 0 0 1 1 *","@weekly":"0 0 0 * * 1","@weekday":"0 0 0 * * 2-6"};
sage.cron.Spec.next=function(a,b){return b.next(a)};sage.cron.Spec.parse=function(a){return new sage.cron.Spec(a)};var cron={js:{}};goog.exportSymbol("Cron",sage.cron.Scheduler);goog.exportSymbol("Cron.prototype.start",sage.cron.Scheduler.prototype.start);goog.exportSymbol("Cron.prototype.stop",sage.cron.Scheduler.prototype.stop);goog.exportSymbol("Cron.prototype.check",sage.cron.Scheduler.prototype.check);goog.exportSymbol("Cron.prototype.add",sage.cron.Scheduler.prototype.add);goog.exportSymbol("Cron.Spec",sage.cron.Spec);goog.exportSymbol("Cron.Spec.prototype.next",sage.cron.Spec.prototype.next);
goog.exportSymbol("Cron.Spec.next",sage.cron.Spec.next);goog.exportSymbol("Cron.Spec.parse",sage.cron.Spec.parse);goog.exportSymbol("Cron.Job",sage.cron.Job);
